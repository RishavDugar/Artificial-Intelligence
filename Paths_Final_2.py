...
ev = Cars(noCar)
q = np.genfromtxt(r"C:\Users\POWER\Downloads\EV.csv", delimiter=',')
q = q.tolist()
ev.stats = q

r = np.genfromtxt(r"C:\Users\POWER\Downloads\Nodes.csv", delimiter=',')
r = r.tolist()
r[0][0] = 0
            ...
            if charging_end_time[id][i] < charging_start_time[max_time_car][i] :
              ...
              if charging_start_time[id][i] < charging_start_time[max_time_car][i]:
                ...
                for k in range(len(path[id])) :
                  if path[id][k] == i :
                    count = 1
                    charging_end_time[id][i] = charging_end_time[id][i] + min(charging_end_time[max_time_car][i]-charging_start_time[max_time_car][i], abs(charging_end_time[id][i]-charging_start_time[max_time_car][i]))
                  if count>0 :
                    time[id][k] = time[id][k] + min(charging_end_time[max_time_car][i]-charging_start_time[max_time_car][i], abs(charging_end_time[id][i]-charging_start_time[max_time_car][i]))
                visited[i][j_val] = -1
                return 1
                ...
                for k in range(len(path[id])) :
                  if path[id][k] == i :
                    count = 1
                    charging_start_time[id][i] = charging_start_time[id][i] + charging_end_time[max_time_car][i]-charging_start_time[id][i]
                    charging_end_time[id][i] = charging_end_time[id][i] + charging_end_time[max_time_car][i]-charging_start_time[id][i]
                  if count>0 :
                    time[id][k] = time[id][k] + charging_end_time[max_time_car][i]-charging_start_time[id][i]
                visited[i][j_val] = -1
                return 1
